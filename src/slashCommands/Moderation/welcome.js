...
module.exports = {
    ...
    options:
        [
            {		
                name: "info",
                description: "See if welcome message is set or not",
                type: 1,
            },
            {		
                name: "set",
                description: "Enable welcome message and set a role to give",
                type: 1,
                options: [
                    {
                        name: "channel",
    ...
    run: async (client, interaction) => {
		...
		if (interaction.options.getSubcommand() === 'set') {
			...
			if(!res) {
				let embed = new EmbedBuilder()
					.setDescription(`Welcome message is already turned off`)
					.setColor(embedError)
